/**
 * ============================================================================
 * 長固 ERP 系統 - 前端核心運算邏輯 (GitHub Pages 適用版)
 * ============================================================================
 */

// 🔴 系統 API 端點 (待後端部署後更新此網址)
...
async function callApi(action, payload = {}) {
    ...
    try {
        const response = await fetch(API_URL, {
            method: 'POST',
            redirect: 'follow', // 解決 GAS CORS 轉址問題的關鍵
            headers: {
...
function renderEmailSettings() {
    ...
    emailSettingsData.list.forEach((item, idx) => {
        ...
        <div class="form-check">
            ...
            <label class="form-check-label fw-bold text-dark" for="cb_email_${idx}">
            ${item.email} <span class="badge bg-secondary ms-1">${item.memo || ''}</span>
            </label>
        </div>`;
    });
...
function saveEmailSettings() {
    ...
    showLoading("儲存設定中...");
    callApi('saveReportEmails', { selectedEmails: selected })
        .then(res => {
            hideLoading();
            showToast("💾 收件信箱設定已儲存");
        })
...
function triggerManualReport() {
    ...
    callApi('sendPendingOrdersReport', {})
        .then(res => {
            hideLoading();
            alert(res.msg);
        })
...
function recheckAiItems() {
    ...
    const html = aiTempData.items.map(i => {
        let internalCode = "";
        if (clientName) {
            const p = globalCatalog.find(x => x.clientName === clientName && x.productName === i.name);
            if(p) internalCode = p.internalCode || p.assetCode || "";
        }
        let codeDisplay = `<span class="badge bg-secondary ms-1">醫院資材碼: ${i.code || '無'}</span>`;
        if (internalCode) codeDisplay += `<span class="badge bg-info text-dark ms-1">長固代號: ${internalCode}</span>`;
        
        return `<div class="p-2 border rounded mb-2 bg-white"><div class="fw-bold text-dark">${i.name} <br> ${codeDisplay}</div><div class="text-danger fw-bold mt-1">數量: ${i.qty}</div></div>`;
    }).join('');
...
function saveOrderAI() {
    ...
    globalOrders.unshift({ 
        rowIdx: 9999, 
        time: Date.now(), 
        client: aiTempData.clientName, 
        orderNo: aiTempData.orderNo, 
        dept: aiTempData.department, 
        status: "待出貨", 
        jsonStr: JSON.stringify(aiTempData.items), 
        deadline: aiTempData.deadline 
    });
...
function renderOrderList() {
    ...
    pending.forEach(o => {
        ...
        let displayItems = items.map(i => {
            ...
            if (invoicedQty === 0) return `<div>${i.name} <span class="badge bg-secondary">x${i.qty}</span></div>`;
            else return `<div>${i.name} <br><small class="text-primary fw-bold">需求: ${i.qty} | 已開: ${invoicedQty} | 剩餘待開: <span class="text-danger">${remaining}</span></small></div>`;
...
function addOrderManualItemRow(itemData = null) {
    ...
    <div class="item-row p-3 mb-2 bg-light border shadow-sm" id="${rowId}">
        ...
        <div class="row g-2">
        <div class="col-4">
            <label class="form-label small fw-bold text-muted mb-1">醫院資材碼</label>
            <input type="text" class="form-control form-control-sm bg-white text-secondary" id="ordM_code_${rowId}" value="${escapeQuotes(itemData?itemData.code:'')}" readonly placeholder="自動匹配">
        </div>
        <div class="col-4">
            <label class="form-label small fw-bold text-muted mb-1 text-info">長固代號</label>
            <input type="text" class="form-control form-control-sm bg-white text-info fw-bold" id="ordM_intcode_${rowId}" value="${escapeQuotes(itemData?itemData.internalCode:'')}" readonly placeholder="內部代號">
        </div>
        <div class="col-4">
            <label class="form-label small fw-bold text-muted mb-1">數量</label>
            <input type="number" class="form-control form-control-sm fw-bold text-primary fs-6" id="ordM_qty_${rowId}" value="${itemData?itemData.qty:''}" min="0" step="any" inputmode="numeric" onchange="updateOrderManualQty('${rowId}', this.value)" placeholder="輸入數量">
        </div>
        </div>
    </div>`;
...
function selectProductForOrderManual(rowId, prodName) {
    ...
    const p = globalCatalog.find(x => x.clientName === clientName && x.productName === prodName);
    if(!p) return;
    document.getElementById(`ordM_name_${rowId}`).value = p.productName;
    document.getElementById(`ordM_code_${rowId}`).value = p.assetCode || '';
    document.getElementById(`ordM_intcode_${rowId}`).value = p.internalCode || '';
    
    const item = currentOrderManualItems.find(x => x.id === rowId);
    if(item) {
        item.name = p.productName;
        item.code = p.assetCode || '';
        item.internalCode = p.internalCode || '';
    }
}

function updateOrderManualQty(rowId, qty) {
    const item = currentOrderManualItems.find(x => x.id === rowId);
...
function saveEditOrder() {
    ...
    if(idx) { 
        ...
        globalOrders.unshift({ rowIdx: Date.now(), time: Date.now(), client: c, orderNo: o, dept: d, status: "待出貨", jsonStr: j, deadline: deadline }); 
    }
...
function groupFulfillOrders() {
    ...
    selectedOrderCache.forEach(order => {
        let items = [];
        try { items = JSON.parse(order.jsonStr || '[]'); } catch(e){}
        
        items.forEach(i => {
            let invoicedQty = globalSalesDetails
                .filter(d => d.orderNo === order.orderNo && d.name === i.name && d.shipStatus !== '作廢')
            ...
            if (remaining > 0) {
                ...
                let unit = p ? p.unit : '式';
                let isMatch = p ? true : false;
                let internalCode = p ? (p.internalCode || p.assetCode) : '';
                
                currentInvoiceData.items.push({
                    id: rowId,
                    product: {productName: i.name, unit: unit, price: price, internalCode: internalCode},
                    qty: remaining,
                ...
                const html = `<div class="item-row border-primary" id="${rowId}">
                    ...
                    <div class="row g-2">
                        <div class="col-6">
                            <label class="form-label fw-bold small">本次開立數量 (可修改)</label>
                            <input type="number" class="form-control" id="qty_${rowId}" value="${remaining}" min="0" step="any" onchange="updateInvQty('${rowId}', this.value)">
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-bold small">歸屬訂單 / 單位</label>
                            <input type="text" class="form-control bg-light text-secondary" value="${escapeQuotes(remarkHint)}" readonly>
                        </div>
                    </div>
                </div>`;
...
function selectProductForInv(rowId, prodName) {
    ...
    document.getElementById(`prodInput_${rowId}`).value = prodName; 
    document.getElementById(`prodInfo_${rowId}`).innerHTML = `單價: $${p.price.toFixed(3)} | 單位: ${p.unit}`; 
    const item = currentInvoiceData.items.find(x => x.id === rowId); 
    if(item) item.product = p;
}

function updateInvQty(rowId, qty) { 
    const item = currentInvoiceData.items.find(x => x.id === rowId); 
    if(item) item.qty = Math.max(0, parseFloat(qty) || 0); 
}
...
function generatePreview() {
    try {
        ...
        if(validItems.length === 0) {
            alert('請完整選擇品項並輸入大於零的數量！');
            return;
        }
        
        let totalWithTax = 0; 
        const prevBody = document.getElementById('prevTableBody'); 
        if(!prevBody) throw new Error("無法找到表格主體");
        prevBody.innerHTML = '';
        
        const invOrderNoEl = document.getElementById('invOrderNo');
        const orderNoStr = invOrderNoEl ? invOrderNoEl.value : '';
        
        const invDateEl = document.getElementById('invDate');
        const invDateStr = invDateEl && invDateEl.value ? invDateEl.value : getTodayStr();
        
        validItems.forEach((item, idx) => { 
            let price = Math.max(0, parseFloat(item.product.price) || 0);
            let qty = Math.max(0, parseFloat(item.qty) || 0);
            let unit = item.product.unit || '式';
            let name = item.product.productName || '未知品項';

            const sub = price * qty; 
            totalWithTax += sub; 
            
            let defaultOrderStr = orderNoStr ? orderNoStr : '';
            let oRef = item.orderRef ? item.orderRef : (idx === 0 && defaultOrderStr ? defaultOrderStr : '');
            let dRef = item.deptRef ? item.deptRef : '';
            let remark = `${oRef} ${dRef}`.trim();
        ...
        const net = Math.round(totalWithTax / 1.05); 
        const tax = totalWithTax - net;
        
        currentInvoiceData.finalNet = net; 
        ...
        const paperNoEl = document.getElementById('invPaperNo');
        setSafeText('prevPaperNo', (paperNoEl ? paperNoEl.value.toUpperCase() : '無') || '無');
        setSafeText('prevInvDate', invDateStr);
        setSafeText('prevNet', net.toLocaleString());
        setSafeText('prevTax', tax.toLocaleString());
        setSafeText('prevTotal', totalWithTax.toLocaleString());
        
        goStep(4);
    } catch(err) {
        alert("預覽結算時發生錯誤: " + err.message);
    }
}
...
function submitInvoiceOptimistic() {
    ...
    const payloadItems = currentInvoiceData.validItems.map(i => ({ 
        ...
        internalCode: i.product.internalCode || i.product.assetCode || "", 
        qty: i.qty, 
        unit: i.product.unit, 
    ...
    const payload = { 
        invDate: invDateTimestamp, 
        staff: myName, 
        clientName: currentInvoiceData.clientName, 
        taxId: currentInvoiceData.taxId, 
        netTotal: currentInvoiceData.finalNet, 
        tax: currentInvoiceData.finalTax, 
        totalWithTax: currentInvoiceData.finalTotal, 
    ...
    globalHistory.unshift({ 
        rowIdx: 9999, 
        time: invDateTimestamp, 
        staff: myName, 
        client: payload.clientName, 
        taxId: payload.taxId, 
        net: payload.netTotal, 
        tax: payload.tax, 
        total: payload.totalWithTax, 
        details: payload.detailsStr, 
    ...
    payloadItems.forEach(pi => {
        ...
        globalSalesDetails.unshift({
            rowIdx: tempIdx, time: invDateTimestamp, paperNo: paperNo, 
            client: payload.clientName, orderNo: orderNo, name: pi.name, 
            qty: pi.qty, shippedQty: 0, shipStatus: '待出貨'
        });
    });
    ...
    const tbody = document.getElementById('visTbody'); 
    tbody.innerHTML = '';
    for(let i=0; i < Math.max(currentInvoiceData.validItems.length, 5); i++) {
        if(i < currentInvoiceData.validItems.length) {
            ...
            const unTaxedS = Math.round(item.qty * (item.product.price / 1.05));
            const remark = item.formattedRemark;
            tbody.innerHTML += `<tr><td class="text-start highlight-data">${item.product.productName}</td><td class="highlight-data">${item.qty} ${item.product.unit}</td><td class="text-end highlight-data">${unTaxedP}</td><td class="text-end highlight-data">${unTaxedS.toLocaleString()}</td><td class="highlight-data" style="font-size:0.8rem;">${remark}</td></tr>`;
        } else { 
...
function triggerSyncAssetCodes() {
    ...
    callApi('syncAssetCodesToInventory', {})
        .then(res => {
            hideLoading();
            if(res.success) { 
                showToast(`✅ 同步完成！共更新了 ${res.count} 筆資材碼資料。`); 
                refreshData(); 
            } else { 
                alert(res.msg); 
            }
...
function renderInvLogs() {
    ...
    if(fStart) {
        const startT = new Date(fStart).setHours(0,0,0,0);
        arr = arr.filter(l => new Date(l.time).setHours(0,0,0,0) >= startT);
    }
    if(fEnd) {
        const endT = new Date(fEnd).setHours(23,59,59,999);
        arr = arr.filter(l => new Date(l.time).getTime() <= endT);
    }
    ...
    c.innerHTML = arr.map(l => {
        ...
        return `<div class="p-3 bg-white border rounded mb-2 shadow-sm d-flex justify-content-between align-items-center">
            <div>
                <div class="fw-bold text-dark">${l.name} <span class="badge ${isAdd?'bg-success':'bg-danger'}">${l.type}</span></div>
                <div class="small text-muted mt-1">${dateStr} | ${l.staff}</div>
                <div class="small text-secondary mt-1">${l.memo}</div>
            </div>
            <div class="text-end">
                <div class="fs-5 fw-bold ${isAdd?'text-success':'text-danger'}">${isAdd?'+':''}${l.qtyChange}</div>
                <div class="small text-muted">結存: ${l.newQty}</div>
            </div>
        </div>`;
    }).join('');
}
...
function saveInventoryAdjust() {
    ...
    const payload = { 
        ...
        cost: cost, supplier: sup, lot: lot, expiry: exp, type: type, memo: memo, staff: myName 
    };
        ...
        return `<div class="item-row bg-white shadow-sm mb-2">
            <div class="d-flex justify-content-between mb-2">
                <div>
                    ...
                    <div class="small text-muted mt-1">${s.client} | 發票: ${s.paperNo}</div>
                </div>
                <button class="btn btn-sm btn-primary fw-bold" onclick="openShipModal(${s.rowIdx})">出貨</button>
            </div>
            <div class="d-flex gap-3 small">
                <span class="text-secondary">總訂購: ${s.qty}</span>
                <span class="text-success">已出: ${s.shippedQty}</span>
                <span class="text-danger fw-bold">欠貨: ${remain}</span>
            </div>
        </div>`;
    }).join('');
}
...
function confirmShipment() {
    ...
    s.shippedQty += qty; 
    if(s.shippedQty >= s.qty) s.shipStatus = '已結案'; 
    else s.shipStatus = '部分出貨';
...
function renderHistory() {
    ...
    c.innerHTML = filtered.map(h => {
        ...
        if(isEdited && !isVoid) {
            const safeLog = escapeQuotes(JSON.parse(h.historyLog).join('\\n'));
        ...
        return `<div class="item-row bg-white shadow-sm p-3 ${isVoid?'status-void':''}">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>${!isVoid ? `<button class="btn btn-sm btn-outline-danger me-1" onclick="voidInv(${h.rowIdx}, '${escapeQuotes(h.paperNo)}')">作廢</button><button class="btn btn-sm btn-outline-secondary" onclick="openEditInv(${h.rowIdx})">編輯</button>` : ''}</div>
...
function saveEditInvoice() {
    ...
    const h = globalHistory.find(x=>x.rowIdx === idx);
    const data = { 
        client: h.client, taxId: h.taxId, 
        paperNo: document.getElementById('e_invPaper').value.toUpperCase(), 
        orderNo: document.getElementById('e_invOrder').value, 
        net: document.getElementById('e_invNet').value, 
        tax: Math.round(document.getElementById('e_invTotal').value - document.getElementById('e_invNet').value), 
        total: document.getElementById('e_invTotal').value, 
        details: document.getElementById('e_invDetails').value 
    };
...
function setReportMonth() { 
    const val = document.getElementById('repMonthPicker').value; if(!val) return; 
...
function exportReportToEmail() {
    ...
    let rCount=0, rNet=0, rTax=0, rTotal=0; 
    const clientStats = {}; 
    const details = []; 
    const lines = [];
    ...
    const payload = { 
        email: email, 
        dateRange: `${sVal} ~ ${eVal}`, 
        summary: { count: rCount, net: rNet, tax: rTax, total: rTotal }, 
        clientStats: Object.keys(clientStats).map(k=>({name:k, total:clientStats[k]})).sort((a,b)=>b.total-a.total), 
        details: details.reverse(), 
        lineItems: lines.reverse() 
    };
    
    callApi('exportExcelReport', payload)
        .then(res => {
            hideLoading(); 
            alert(`✅ 報表已寄送至 ${email}`);
        })
...
function openAdminItemModal(idx) { 
    ...
    if(idx) { 
        ...
        ipt.onclick = null; 
        ipt.classList.remove('fake-input-btn'); 
        ...
        ipt.value = ''; 
        ipt.onclick = triggerItemClientSelect; 
        ipt.classList.add('fake-input-btn'); 
