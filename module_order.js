/**
 * ============================================================================
 * 模組 2：訂單辨識建檔與信件設定 (module_order.js)
 * ============================================================================
 */
...
window.triggerManualReport = function() {
    ...
    }).catch(err => { hideLoading(); alert("發送失敗：" + err.message); });
};
...
window.processOrderUpload = function(input) {
    ...
    const file = input.files[0]; const mimeType = file.type || 'application/pdf'; const reader = new FileReader();
...
window.recheckAiItems = function() {
    ...
    const html = aiTempData.items.map((i, index) => {
        ...
        return `<div class="p-3 border rounded mb-2 bg-white shadow-sm">
            ...
            <input type="text" class="form-control form-control-sm fw-bold text-dark mb-2" value="${escapeQuotes(i.name)}" oninput="updateAiItemName(${index}, this.value)">
            <div class="d-flex justify-content-between align-items-center mt-2">
                <div>${codeDisplay}</div>
                <div class="d-flex align-items-center">
                    <label class="form-label small fw-bold text-danger mb-0 me-2">數量:</label>
                    <input type="number" class="form-control form-control-sm text-danger fw-bold text-center" style="width: 70px;" value="${i.qty}" min="0" step="any" oninput="updateAiItemQty(${index}, this.value)">
                </div>
            </div>
        </div>`;
    }).join('');
...
window.saveOrderAI = function() {
    ...
    globalOrders.unshift({ 
        rowIdx: 9999, time: Date.now(), client: aiTempData.clientName, orderNo: aiTempData.orderNo, 
        dept: aiTempData.department, status: "待出貨", jsonStr: JSON.stringify(aiTempData.items), 
        deadline: aiTempData.deadline, source: aiTempData.source, mailUrl: aiTempData.mailUrl 
    });
...
window.renderOrderList = debounce(function() {
    ...
    if (searchTerm) {
        pending = pending.filter(o => (o.client && o.client.toLowerCase().includes(searchTerm)) || 
    ...
    pending.forEach(o => {
        ...
        let displayItems = items.map(i => {
            ...
            if (invoicedQty === 0) {
                return `<div>${i.name} ${intCodeBadge} <span class="badge bg-secondary">x${i.qty}</span></div>`; 
            } else {
                return `<div>${i.name} ${intCodeBadge} <br><small class="text-primary fw-bold">需求: ${i.qty} | 已開: ${invoicedQty} | 剩餘: <span class="text-danger">${remaining}</span></small></div>`;
            }
